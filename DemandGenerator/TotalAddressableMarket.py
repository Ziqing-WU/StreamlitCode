...
if not csv_file:
    ...
    with open(f"data_precharged/{current_year}_vehicle_list_TM.pickle", "rb") as f:
        dict_input = pickle.load(f)
    df = dict_input["Dataframe"]
...
with st.expander("Technical Constraints"):
    ...
    min_age_tech, max_age_tech = st.select_slider('Vehicle Age Range',
                                                    options=np.array([i for i in range(1,26)]),
                                                    value=[5,15])
...
with col2:
    st.write(df.head(10))
    dict_output = {"Parameters": param, "Dataframe": df}
    if st.button("Download parameters and dataset in the total addressable market"):
        with open(f"data_precharged/{current_year}_vehicle_list_TAM.pickle", "wb") as f:
